...
def is_valid(f):
    ...
    if hasattr(f, 'save'):  # for werkzeug.datastructures.FileStorage objects
        with tempfile.NamedTemporaryFile() as t:
            # TODO: pipe contents directly to cmd
            contents = f.read()
            t.write(contents)
            t.flush()
            f.seek(0)
...
def from_srt(input_f, output_f):
    """Take an input SRT file or filename and write out VTT contents to the
    given output file or filename.
    """
    with vtt_open(input_f, 'r') as f:
        ...
        contents = contents[3:] if contents[:3] == codecs.BOM_UTF8 else contents  # noqa: E501
...
def shift_time(input_f, output_f, offset_secs):
    """Take the time codes in the input file and add (or remove) the offset in
    ...
    def string_from_datetime(time):
        return (time + DELTA).strftime(FORMAT)[:EXTRA_PAD_STRIP]
    ...
    rx = re.compile(r"^(\d{2}:\d{2}:\d{2}\.\d{3}) --> (\d{2}:\d{2}:\d{2}\.\d{3})\s*$")  # noqa: E501
