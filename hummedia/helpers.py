...
class OAuthProvider():
    ...
    def __init__(self, providerService):
        ...
        self.remote_app = self.oauth.remote_app(providerService, **kwargs)
...
class Resource(object):
    ...
    if not model:
        raise NoModelException("You must declare the model for the resource.")
    ...
    def acl_read_check(self, obj, username, allowed, is_nested_obj=False,
        ...
        if not self.override_only:
            return any([role == "faculty",
                        obj["@graph"].get("dc:coverage") in allowed,
                        username in obj["@graph"].get("dc:rights", {}).get("read", []),  # noqa: E501
                        not is_nested_obj and obj["@graph"].get("dc:creator") == username])  # noqa: E501
        return False
    ...
    def acl_write_check(self, bundle=None):
        ...
        if atts['superuser'] or (atts['role'] == 'faculty' and not bundle):
            return True
        if bundle:
            if bundle["@graph"].get("dc:creator") == atts['username'] or atts['username'] in bundle['@graph']["dc:rights"]["write"]:  # noqa: E501
                return True
        return False
    ...
    def auth_filter(self, bundle=None):
        ...
        if not atts['username']:
            ...
            filtered_bundle = self.acl_filter(["public", "BYU"],
                                              atts['username'], atts['role'],
                                              bundle)
        else:
            filtered_bundle = bundle if bundle else self.bundle
        return filtered_bundle

    def acl_filter(self, allowed=["public"], username="unauth", role=None,
                   bundle=None):
        if not bundle:
            bundle = self.bundle
    ...
    def set_resource(self):
        self.bundle["resource"] = uri_pattern(str(self.bundle["_id"]),
                                              APIHOST + "/" + self.endpoint)

    def set_attrs(self):
        for (k, v) in self.request.json.items():
            if k in self.model.structure and k not in self.disallowed_atts:
                if isinstance(self.model.structure[k], int):
                    self.bundle[k] = int(v)
                elif isinstance(self.model.structure[k], float):
                    self.bundle[k] = float(v)
                elif isinstance(self.model.structure[k], unicode):
                    self.bundle[k] = unicode(v)
                elif isinstance(self.model.structure[k], list):
                    self.bundle[k] = []
                    for i in v:
    ...
    def dispatch(self, id):
        methods = {"GET": self.get, "POST": self.post, "PUT": self.put,
...
def get_enrollments():
    ...
    tokenRes = requests.post('https://api.byu.edu/token',
    ...
    res = requests.get('https://api.byu.edu:443/domains/legacy/academic/registration/enrollment/v1/studentschedule/personid/' + get_userid() + '/' + getCurrentSem(), headers=headers)  # noqa: E501
    ...
    for course in content["class_list"]:
        courses.append(" ".join((course['subject_area'],
...
def is_enrolled(obj):
    try:
        return not set(get_enrollments()).isdisjoint(obj["@graph"]["dc:relation"])  # noqa: E501
    ...
    def decorator(f):
        def wrapped_function(*args, **kwargs):
            ...
            if nocache:
                ...
                h['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0'  # noqa: E501
            return resp
...
def mongo_jsonify(obj):
    return Response(json.dumps(obj, cls=mongokitJSON), status=200,
                    mimetype="application/json")


def build_html_table(videos, fields, headings):
    lines = list()
    heading_line = '<tr>%s</tr>' % ''.join('<th>%s</th>' % str(h)
                                           for h in headings)
...
def bundle_404():
    return Response("The object was not found", status=404,
                    mimetype="text/plain")


def endpoint_404():
    return Response("That service does not exist", status=404,
                    mimetype="text/plain")
...
def action_401():
    return Response("You do not have permission to perform that action.",
...
def getYtThumbs(ids=[]):
    ...
    if len(ids):
        ...
        for vid in j["items"]:
            ytThumbs[vid["id"]] = {"poster": vid["snippet"]["thumbnails"]["high"]["url"],  # noqa: E501
                                   "thumb": vid["snippet"]["thumbnails"]["default"]["url"]}  # noqa: E501
    return ytThumbs
...
def getCurrentSem():
    ...
    url = "https://ws.byu.edu/rest/v1/academic/controls/controldatesws/asofdate/{0}/semester.json".format(date)  # noqa: E501
    try:
        sem = json.loads(requests.get(url).content)["ControldateswsService"]["response"]["date_list"][0]["year_term"]  # noqa: E501
    except IndexError:
        sem = 5
    return sem
...
def getVideoInfo(filename):
    ...
    cmd = ['avprobe', filename, '-show_streams', '-show_format', '-loglevel',
           'quiet']
    ...
    # ensure the data we have is that for the video. doesn't include audio data
    stream_match = re.search(
        r'\[STREAM\]\s*(.*?codec_type=video.*?)\s*\[/STREAM\]',
        output,
        re.DOTALL)

    format_match = re.search(r'\[FORMAT\]\s*(.*)\[/FORMAT\]', output,
                             re.DOTALL)

    lines = stream_match.group(1).splitlines() + format_match.group(1).splitlines()  # noqa: E501
    data = {line.split('=')[0].strip(): line.split('=')[1].strip() for line in lines}  # noqa: E501
