...
class User(Document):
    ...
    structure = {"username": basestring,
                 ...
                 "oauth": {"google": {"id": basestring, "email": basestring,
                                      "access_token": list},
                           "facebook": dict,
                           "twitter": dict}}
    required_fields = ["username", "email"]
    default_values = {"username": "", "email": "", "preferredLanguage": "en",
                      "role": "student", "superuser": False, "ta": False}
...
class Clip(Document):
    ...
    __database__ = MONGODB_DB
    use_schemaless = True
    structure = {  # '_id': basestring,  # unique id of clip
                 'userid': basestring,  # ID of user
                 'mediaid': basestring,  # ID of video
                 'name': basestring,  # name of clip
                 'start': float,  # start time (seconds)
                 'end': float}  # end time (seconds)
    # 'end' is optional - clips will continue from bookmark if undefined.
...
class AnnotationList(Document):
    ...
    structure = {"@context": dict,
                 "@graph": {"pid": basestring,
                            ...
                            "dc:rights": {"read": list,
                                          "write": list}}}
    required_fields = ["@context", "@graph.dc:creator", "@graph.dc:date",
                       "@graph.dc:title"]
    default_values = {"@context": {"dc": "http://purl.org/dc/",
                                   "oa": "http://www.w3.org/ns/oa#",
                                   "oax": "http://www.w3.org/ns/openannotation/extension/",  # noqa: E501
                      ...
                      "@graph.vcp:playSettings": {"vcp:showTranscript": False,
                                                  "vcp:audioOffset": 0,
                                                  "vcp:videoOffset": 0,
                                                  "vcp:videoCrop": 0,
...
class AssetGroup(Document):
    ...
    structure = {"@context": dict,
                 "@graph": {"pid": basestring,
                            ...
                            "dc:date": IsoDate(),
                            "dc:coverage": basestring,
                            "dc:rights": {"read": list, "write": list}}}
    required_fields = ["@context", "@graph.dc:type", "@graph.dc:creator",
                       "@graph.dc:date", "@graph.dc:title"]
    default_values = {"@context": {"authorized": False,
                                   "dc": "http://purl.org/dc/elements/1.1/",
                                   "hummedia": "http://humanities.byu.edu/hummedia/",  # noqa: E501
                                   "dc:identifier": "@id",
                                   "dc:type": "@type"},
                      "@graph.dc:creator": "Hummedia",
                      "@graph.dc:relation": [],
...
class Video(Document):
    ...
    structure = {"@context": dict,
                 "@graph": {"dc:creator": basestring,
                            ...
                            "ma:isCopyrightedBy": {"@id": basestring,
                                                   "name": unicode},
                            "ma:date": int,
                            "ma:description": unicode,
                            "ma:duration": int,
                            "ma:features": [{"@id": basestring,
                                             "name": unicode}],
                            "ma:frameHeight": int,
                            "ma:frameRate": OR(float, int),
                            "ma:frameSizeUnit": basestring,
                            "ma:frameWidth": int,
                            "ma:hasContributor": [{"@id": basestring,
                                                   "name": unicode}],
                            "ma:hasGenre": {"@id": basestring,
                                            "name": unicode},
                            ...
                            "ma:locator": [{"location": basestring,
                                            "ma:hasFormat": basestring,
                                            "ma:hasCompression": {"@id": basestring,  # noqa: E501
                                                                  "name": basestring}}],  # noqa: E501
                            "ma:title": unicode}}
    required_fields = ["@context", "@graph.dc:creator", "@graph.dc:date",
                       "@graph.dc:type", "@graph.ma:date",
                       "@graph.ma:hasLanguage", "@graph.ma:title",
                       "@graph.ma:duration"]
    default_values = {"@context": {"dc": "http://purl.org/dc/elements/1.1/",
                                   "dc:identifier": "@id",
                                   "dc:type": "@type",
    ...
    def make_part(self, vid, host, part):
        ...
        thepart = {"ma:title": vid["ma:title"], "pid": vid.get("pid"),
                   "resource": resource}
        ...
        if part != "snippet":
            ...
            for location in vid["ma:locator"]:
                ...
                if resolve_type(vid["dc:type"]) == "humvideo":
                    ...
                    thumb = uri_pattern(location["@id"] + "_thumb.jpg",
                                        HOST + "/posters")
                    thepart["ma:image"].append({"poster": poster,
                                                "thumb": thumb})
            ...
            for att in ["ma:date", "ma:description", "ma:hasLanguage",
                        "ma:hasPolicy", "ma:isMemberOf"]:
                thepart[att] = vid.get(att)
            for annot in thepart["ma:isMemberOf"]:
                coll = connection[MONGODB_DB].assetgroups.find_one({"_id": annot["@id"]})  # noqa: E501
                if coll is not None:
