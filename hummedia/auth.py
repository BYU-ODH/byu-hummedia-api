...
from helpers import get_enrollments
from helpers import plain_resp
# from interfaces import ItsdangerousSessionInterface
from models import connection
from providers import GoogleOAuth2
from providers import CasAuth
...
oAuthService = provider_lookup["google"]("google")  # eventually we can have multiple providers ... for now, it's hard coded  # noqa: E501
...
def get_profile():
    atts = {}
    for att in ['username', 'userid', 'role', 'superuser', 'fullname',
                'preferredLanguage', 'ta']:
        atts[att] = session.get(att)
    if (atts['username'] is None
            or atts['username'] == "") and session.get('oauth'):
...
def get_user_from_oauth(provider, atts, access_token=None):
    ...
    if user is None:
        session["oauth"] = {"provider": provider, "id": atts['id'],
                            "access_token": access_token,
                            "email": atts['email']}
        user = {"username": "", "role": "", "superuser": False, "fullname": "",
                "preferredLanguage": "en", "ta": False}
    return {"username": user['username'], "role": user['role'],
            "superuser": user["superuser"], "fullname": user["fullname"],
            "preferredLanguage": user["preferredLanguage"],
            "ta": user.get("ta", False)}
...
def get_user_from_cas(netid=None, atts=None):
    ...
    faculty_positions = ["activeFulltimeInstructor",
    ...
    if "provider" in oauth:
        if user['oauth'][oauth['provider']]['id'] is None:
            user["oauth"][oauth['provider']] = {"id": oauth['id'],
                                                "email": oauth['email'],
                                                "access_token": oauth['access_token']}  # noqa: E501
...
@crossdomain(origin=config.CROSS_DOMAIN_HOSTS,
             headers=['Origin', 'x-requested-with', 'accept', 'Content-Type',
                      'Authorization'],
             credentials=True)
def auth_redirect(provider="cas", authUser=None, proxyUser=None):
    if provider == "google":
        ...
        if th:
            ...
            if verification:
                user = get_user_from_oauth(provider, verification,
                                           get_access_token())
                if user['username'] != "":
                    set_session_vars(user)
                elif get_user():
...
@app.route('/account/login/<providerService>/<proxyUser>',
           methods=['GET', 'OPTIONS'])
@crossdomain(origin=config.CROSS_DOMAIN_HOSTS,
             headers=['Origin', 'x-requested-with', 'accept', 'Content-Type',
                      'Authorization'],
...
def apiLogin(providerService="cas", proxyUser=None):
    ...
    if providerService == "google":
        if get_access_token():
            ...
            return provider.authorize(callback=config.APIHOST + config.GOOGLE_REDIRECT_URI)  # noqa: E501
    elif providerService in ["authUser", "proxy"]:
        return auth_redirect(providerService,
                             request.headers.get("Authorization"), proxyUser)
    else:
...
def apiLogout():
    ...
    request.args.get("r", get_redirect_url())
    session.clear()
...
def authorized(resp):
    ...
    if provider == "google":
        ...
        if "ticket" in request.args:
            user, atts = cas.verify_ticket(request.args.get('ticket'),
                                           config.APIHOST + config.REDIRECT_URI)  # noqa: E501
            if user:
